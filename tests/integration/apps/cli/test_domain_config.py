    ...
    def test_saving_and_fetching_config(
        ...
        for dc in domain_configs:
            handler.save_domain_config(
                SaveDomainConfigCommand(config=dc)
            )

        # When: manager fetches configs
        found_domain_configs = (
            manager.list_domain_configs()
        )
        ...
        domain_names = [
            config.domain
            for config in found_domain_configs.values()
        ]
        ...
        handler.save_domain_config(
            SaveDomainConfigCommand(config=domain_config_3)
        )
