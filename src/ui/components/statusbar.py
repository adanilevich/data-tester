    ...
    def render(self) -> ui.footer:
        ...
        with ui.footer().classes(
            ...
            with ui.row().classes("items-center gap-10"):
                ...
                if domain:
                    self._chip("Config", self.controller.get_domain_config_status(domain))
                    self._chip("Testsets", self.controller.get_testset_status(domain))
                    self._chip(
                        "Testobjects", self.controller.get_testobjects_status(domain)
                    )
                    self._chip("Testruns", self.controller.get_testruns_status(domain))
                    self._chip("Specs", self.controller.get_specs_status(domain))
